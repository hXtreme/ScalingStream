...
class SentenceSource(
    ...
    private val words: List<String> = file.readText().split(" ")

    private var num: Int = 0

    fun generateSentence(): String {
        return (0..sentenceLength).joinToString(separator = " ") { words.random() }
    }

    fun generator(): String? {
        return if (num++ < numRecords) generateSentence() else CONSTANTS.DONE_MARKER
    }
}

fun main(args: Array<String>) {
    val file = File(args.getOrElse(0) { "./README.md" })
    val sentences = SentenceSource(file, 100, 10)
